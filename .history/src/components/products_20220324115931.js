...
export const Products = ({ currentItems }) => {
  ...
  const handleShow = (value) =>{
    ...
    return (
        ...
        currentItems.map(((item , key) => (
          ...
          <button onClick={handleShow(key)}>more pics</button>
          {/* <div  style={{display : show ? "block" : "none"}}>
            <img src={item.additional_image_link.split(',')}/>
          </div> */}

          <Modal id={key} show={show === key} onHide={handleShow(0)}>
            
            <img src={item.additional_image_link.split(',')}/>
            <ModalFooter>
            <button onClick={handleShow(0)} className="close-btn">Cancel</button>
            </ModalFooter>
          </Modal>
          </div>
