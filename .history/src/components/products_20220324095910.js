  ...
  const handleShow = () => setShow(true);
    return (
        ...
        currentItems.map(((item) => (
          ...
          <button onClick={this.handleShow()}>more pics</button>
         


          <Modal key={item.gtin} show={show} onHide={this.handleClose()}>
            
            <img src={item.additional_image_link.split(',')}/>
            <ModalFooter>
            <button onClick={this.handleClose()} className="close-btn">Cancel</button>
            </ModalFooter>
          </Modal>
          </div>
