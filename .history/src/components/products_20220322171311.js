  ...
  const hideModal = () => {
    ...
    return (
        ...
        currentItems.map(((item , index) => (
          ...
          { item[index].additional_image_link ? item[index].additional_image_link.map((item , index) => {
            var arr = item.equipment.split(',');
            const itemsToRender = currentItems.reduce(function(brr, pic){
          ...
          <Modal show={isOpen} onHide={hideModal} key={index}>
            
              <Carousel>
                { itemsToRender.map((img) => {
                    console.log(img);
                    <Carousel.Item>
                      <img
